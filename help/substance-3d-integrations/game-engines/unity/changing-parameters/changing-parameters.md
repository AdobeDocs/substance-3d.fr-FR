...
1. Dans la fenêtre Projet, sélectionnez le logo fichier sbsar du graphe à personnaliser. Le sbsar a le logo vert « SBSAR ».

   ![](../../../assets/screen-shot-2022-03-29-at-2-27-56-pm.png)

## Propriétés procédurales

1. **Générer toutes les sorties** : génère toutes les sorties à partir du fichier sbsar de Substance. Par défaut, seules les sorties utilisées par les shaders standard sont créées.
1. **Générer des mipmaps** : générera des textures mip pour chaque sortie de Substance.
1. **Générateur aléatoire** : ce bouton modifie le générateur aléatoire utilisé par le graphe de Substance de données pour générer les textures. Si vous modifiez cette valeur, la texture calculée sera modifiée en fonction de la valeur initiale.
