...
* Le graphique et la matière correspondante ont à présent le même nom d’objet
* Ajout d’améliorations de la lisibilité de l’interface utilisateur graphique d’habillage d’Unity Pro
* Ajout de la prise en charge des affectations de sortie des matières
* Correction d’un bug lié à la gestion sRVB
* Correction d’un bug en raison duquel un utilisateur pouvait supprimer toutes les instances d’un graphique.
...
* Ajout de boutons de navigation dans l’interface graphique de Substance pour créer des liens vers les graphiques et les matériaux de Substance
* Ajout de nouvelles icônes pour le graphique/matériau/textures de Substance dans l’Explorateur de contenu
* Mise à jour des vignettes de Substance dans le navigateur de contenu
* Suppression du fichier .mat au début des noms des matériaux de Substance
* Possibilité de renommer les graphiques et les matériaux de Substance ajoutée
* Lors de la modification de la résolution du graphique en Substance, la fenêtre contextuelle Appliquer/Rétablir n’apparaîtra plus, forçant l’utilisateur à valider la modification à ce moment-là
...
* Fonctionnalité modifiée des instances de graphique de Substance : les utilisateurs peuvent désormais créer des instances de graphique dans une Substance de données sans être invités à chaque instance créée dans l’interface utilisateur graphique du graphique de Substance

### Scripts :

* Nous avons masqué certaines fonctions non destinées à la prise en charge des scripts
* Fonction ajoutée pour dupliquer les instances de graphiques de Substance via le script : Duplicate()
* Fonction ajoutée pour interroger des informations d&#39;entrée de procédure via C#, qui renvoie un tableau d&#39;éléments &#39;InputProperties&#39; : GetInputProperties()
* Fonction ajoutée pour vérifier si une entrée existe dans un graphique, retourne true/false : HasInput(string inputName)
...
* Le bouton d’Alpha et le curseur d’aperçu Mappage sont manquants dans l’interface utilisateur graphique des textures non Substances après l’importation du module externe Substance
* L’utilisateur doit utiliser des puissances de deux pour définir la résolution d’un graphique de Substance à l’aide d’un script
* Les matériaux de Substance ne sont pas persistants lorsqu’ils sont exportés/importés avec un package Unity
...
* Si vous renommez un graphique en Substance contenant un matériau dans la scène, ce matériau sera supprimé des objets sur lesquels il est placé
