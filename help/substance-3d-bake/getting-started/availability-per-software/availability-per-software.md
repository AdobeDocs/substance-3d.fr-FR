---
helpx_url: "https://helpx.adobe.com/fr/substance-3d-bake/getting-started/availability-per-software.html"
breadcrumb-title: ''
description: Vérifiez quels bakers sont disponibles dans chaque application Substance 3D pour planifier votre flux de baking.
helpx_creative_field: ""
helpx_description: bakers > Getting Started > Availability per software
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Disponibilité par logiciel
user-guide-description: ''
user-guide-title: ''
source-git-commit: 0197b6f5f4e3ed1f2bc0e5576bd5818d04485ed5
workflow-type: tm+mt
source-wordcount: '105'
ht-degree: 28%

---


# Disponibilité par logiciel

Certains bakers peuvent ne pas être disponibles dans tous les logiciels. La liste ci-dessous fait référence à ceux qui sont présents par logiciel.

|  | Substance 3D Designer | Substance Automation Toolkit | Substance 3D Painter |
| --- | --- | --- | --- |
| Occlusion ambiante |  | ■ |  |
| Ambient occlusion du maillage | ■ | ■ | ■ |
| Bents normals du maillage | ■ | ■ | **■** |
| Table des couleurs par maillage | ■ | ■ | ■ |
| Convertir UV en SVG | ■ | ■ |  |
| Courbure |  | ■ | ■ |
| Curvature à partir du maillage | ■ | ■ | ■ |
| Curvature à partir du maillage (obsolète) |  | ■ |  |
| Map height du maillage | ■ | ■ | ■ |
| Map normal du maillage | ■ | ■ | ■ |
| Masque d’opacité par maillage | ■ | ■ | ■ |
| Position | ■ | ■ | ■ |
| Position à partir du maillage | ■ | ■ |  |
| Map thickness du maillage | ■ | ■ | ■ |
| Texture transférée à partir du maillage | ■ | ■ |  |
| Direction dans l&#39;espace monde | ■ | ■ |  |
| Normales de l&#39;espace monde | ■ | ■ | ■ |
