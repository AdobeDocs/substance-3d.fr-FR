...
breadcrumb-title: ''
description: Découvrez comment optimiser la configuration matérielle et la préparation du maillage pour obtenir des performances de bake plus rapides.
...
Les bakers fonctionnent en interne avec des maillages triangulés ; si les modèles 3D (poly faible et élevé) ne sont pas triangulés, les bakers triangulent les maillages eux-mêmes. Ce processus peut prendre beaucoup de temps et augmentera linéairement en fonction de la quantité de polygones contenus dans le modèle. Il est généralement conseillé de trianguler les maillages (en particulier le maillage high poly) afin d&#39;éviter que ce processus ne se produise pendant le bake.
...
Les bakers peuvent utiliser le suréchantillonnage pour effectuer le lissage. Le suréchantillonnage signifie que les bakers convertiront davantage de rayons par pixel afin de lisser le résultat. Le temps de bake peut être considérablement affecté par ce paramètre ; cela est particulièrement vrai pour les bakers où de nombreux rayons sont nécessaires, tels que l’ambient occlusion du baker du maillage.

À titre d’exemple :

* un paramètre AA de 2x2 signifie que le baker convertira 4 fois la quantité initiale de rayons. Pour une texture de 2048\*2048 px, le calcul obtenu équivaut à bake une texture de 4096\*4096px et devrait prendre environ 4 fois plus de temps à calculer.
* un réglage AA de 8x8 signifie que le baker convertira 64 fois la quantité initiale de rayons. Pour une texture de 2048\*2048 px, le temps de calcul résultant équivaut à bake une texture de 16384\*16384px et devrait prendre environ 64 fois plus de temps à calculer.
